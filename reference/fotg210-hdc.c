...
static void __maybe_unused dbg_itd(const char *label,
  ...
  fotg210_dbg(fotg210, "%s[%d] itd %p, next %08x, urb %p\n", label, itd->frame,
              itd, hc32_to_cpu(fotg210, itd->hw_next), itd->urb);

  fotg210_dbg(fotg210, "  trans: %08x %08x %08x %08x %08x %08x %08x %08x\n",
  ...
  fotg210_dbg(fotg210, "  buf:   %08x %08x %08x %08x %08x %08x %08x\n",
  ...
  fotg210_dbg(fotg210, "  index: %d %d %d %d %d %d %d %d\n", itd->index[0],
              itd->index[1], itd->index[2], itd->index[3], itd->index[4],
              itd->index[5], itd->index[6], itd->index[7]);
}

static int __maybe_unused dbg_status_buf(char *buf, unsigned len,
                                         const char *label, u32 status) {
  return scnprintf(
      buf, len, "%s%sstatus %04x%s%s%s%s%s%s%s%s%s%s", label,
      label[0] ? " " : "", status, (status & STS_ASS) ? " Async" : "",
...
static int __maybe_unused dbg_intr_buf(char *buf, unsigned len,
  ...
  return scnprintf(
      buf, len, "%s%sintrenable %02x%s%s%s%s%s%s", label, label[0] ? " " : "",
      enable, (enable & STS_IAA) ? " IAA" : "",
...
static int dbg_command_buf(char *buf, unsigned len, const char *label,
                           u32 command) {
  return scnprintf(
      buf, len, "%s%scommand %07x %s=%d ithresh=%d%s%s%s period=%s%s %s", label,
      label[0] ? " " : "", command, (command & CMD_PARK) ? " park" : "(park)",
...
static char *dbg_port_buf(char *buf, unsigned len, const char *label, int port,
  ...
  scnprintf(buf, len, "%s%sport:%d status %06x %d sig=%s%s%s%s%s%s%s%s", label,
            label[0] ? " " : "", port, status, status >> 25, /*device address */
            sig, (status & PORT_RESET) ? " RESET" : "",
            (status & PORT_SUSPEND) ? " SUSPEND" : "",
            (status & PORT_RESUME) ? " RESUME" : "",
            (status & PORT_PEC) ? " PEC" : "", (status & PORT_PE) ? " PE" : "",
            (status & PORT_CSC) ? " CSC" : "",
...
static void qh_lines(struct fotg210_hcd *fotg210, struct fotg210_qh *qh,
  ...
  temp = scnprintf(
      next, size, "qh/%p dev%d %cs ep%d %08x %08x(%08x%c %s nak%d)", qh,
      scratch & 0x007f, speed_char(scratch), (scratch >> 8) & 0x000f, scratch,
      hc32_to_cpup(fotg210, &hw->hw_info2),
      hc32_to_cpup(fotg210, &hw->hw_token), mark,
      (cpu_to_hc32(fotg210, QTD_TOGGLE) & hw->hw_token) ? "data1" : "data0",
  ...
  list_for_each_entry(td, &qh->qtd_list, qtd_list) {
    ...
    temp = snprintf(next, size, "\n\t%p%c%s len=%d %08x urb %p", td, mark, ({
                    ...
                    (scratch >> 16) & 0x7fff, scratch, td->urb);
    if (size < temp)
      temp = size;
    size -= temp;
    next += temp;
...
static ssize_t fill_async_buffer(struct debug_buffer *buf) {
  ...
  *next = 0;
  ...
  if (fotg210->async_unlink && size > 0) {
    ...
    for (qh = fotg210->async_unlink; size > 0 && qh; qh = qh->unlink_next)
      qh_lines(fotg210, qh, &next, &size);
  }
...
static unsigned output_buf_tds_dir(char *buf, struct fotg210_hcd *fotg210,
  ...
  return scnprintf(buf, size, "(%c%d ep%d%s [%d/%d] q%d p%d)",
                   ...
                   (scratch >> 8) & 0x000f, type, qh->usecs, qh->c_usecs, temp,
                   (scratch >> 16) & 0x7ff);
}
...
static ssize_t fill_periodic_buffer(struct debug_buffer *buf) {
  ...
  for (i = 0; i < fotg210->periodic_size; i++) {
    ...
    do {
      ...
      case Q_TYPE_QH:
        hw = p.qh->hw;
        temp = scnprintf(next, size, " qh%d-%04x/%p", p.qh->period,
                         hc32_to_cpup(fotg210, &hw->hw_info2)
                             /* uframe masks */
                             & (QH_CMASK | QH_SMASK),
                         p.qh);
        ...
        if (temp == seen_count) {
          temp = output_buf_tds_dir(next, fotg210, hw, p.qh, size);
        ...
        break;
      case Q_TYPE_FSTN:
        temp = scnprintf(next, size, " fstn-%8x/%p", p.fstn->hw_prev, p.fstn);
...
static ssize_t fill_registers_buffer(struct debug_buffer *buf) {
  ...
  temp =
      scnprintf(next, size,
                ...
                hcd->self.controller->bus->name, dev_name(hcd->self.controller),
                hcd->product_desc, i >> 8, i & 0x0ff, rh_state_string(fotg210));
  size -= temp;
  next += temp;
...
#ifdef FOTG210_STATS
  temp = scnprintf(next, size, "irq normal %ld err %ld iaa %ld(lost %ld)\n",
...
static ssize_t debug_output(struct file *file, char __user *user_buf,
  ...
  ret = simple_read_from_buffer(user_buf, len, offset, buf->output_buf,
                                buf->count);
...
static inline void create_debug_files(struct fotg210_hcd *fotg210) {
  ...
  debugfs_create_file("periodic", S_IRUGO, root, bus, &debug_periodic_fops);
  debugfs_create_file("registers", S_IRUGO, root, bus, &debug_registers_fops);
}
 ...
 */
static int handshake(struct fotg210_hcd *fotg210, void __iomem *ptr, u32 mask,
                     u32 done, int usec) {
  u32 result;
  int ret;

  ret = readl_poll_timeout_atomic(
      ptr, result, ((result & mask) == done || result == U32_MAX), 1, usec);
...
static int fotg210_halt(struct fotg210_hcd *fotg210) {
  ...
  return handshake(fotg210, &fotg210->regs->status, STS_HALT, STS_HALT,
                   16 * 125);
}
...
static int fotg210_reset(struct fotg210_hcd *fotg210) {
  ...
  retval =
      handshake(fotg210, &fotg210->regs->command, CMD_RESET, 0, 250 * 1000);
  ...
  fotg210->port_c_suspend = fotg210->suspended_ports = fotg210->resuming_ports =
      0;
  return retval;
}
...
static void fotg210_enable_event(struct fotg210_hcd *fotg210, unsigned event,
  ...
  if (event < fotg210->next_hrtimer_event) {
    ...
    hrtimer_start_range_ns(&fotg210->hrtimer, *timeout, NSEC_PER_MSEC,
                           HRTIMER_MODE_ABS);
  }
}
...
static void fotg210_poll_ASS(struct fotg210_hcd *fotg210) {
  ...
  if (want == 0) { /* Stopped */
    ...
    if (fotg210->async_count == 0) {
      ...
      fotg210_enable_event(fotg210, FOTG210_HRTIMER_DISABLE_ASYNC, true);
    }
  }
}
...
static void fotg210_poll_PSS(struct fotg210_hcd *fotg210) {
  ...
  if (want == 0) { /* Stopped */
    ...
    if (fotg210->periodic_count == 0) {
      ...
      /* Turn off the schedule after a while */
      fotg210_enable_event(fotg210, FOTG210_HRTIMER_DISABLE_PERIODIC, true);
    }
  }
}
...
static void start_free_itds(struct fotg210_hcd *fotg210) {
  if (!(fotg210->enabled_hrtimer_events & BIT(FOTG210_HRTIMER_FREE_ITDS))) {
    fotg210->last_itd_to_free =
        list_entry(fotg210->cached_itd_list.prev, struct fotg210_itd, itd_list);
...
static void turn_on_io_watchdog(struct fotg210_hcd *fotg210) {
  ...
  if (fotg210->rh_state != FOTG210_RH_RUNNING ||
      (fotg210->enabled_hrtimer_events & BIT(FOTG210_HRTIMER_IO_WATCHDOG)))
    return;
...
static int fotg210_hub_control(struct usb_hcd *hcd, u16 typeReq, u16 wValue,
  ...
  case ClearPortFeature:
    ...
    case USB_PORT_FEAT_SUSPEND:
      ...
      fotg210->reset_done[wIndex] =
          jiffies + msecs_to_jiffies(USB_RESUME_TIMEOUT);
      break;
  ...
  case GetPortStatus:
    ...
    if (temp & PORT_RESUME) {
      ...
      if (!fotg210->reset_done[wIndex]) {
        /* resume signaling for 20 msec */
        fotg210->reset_done[wIndex] = jiffies + msecs_to_jiffies(20);
      ...
      /* resume completed? */
      else if (time_after_eq(jiffies, fotg210->reset_done[wIndex])) {
        ...
        retval = handshake(fotg210, status_reg, PORT_RESUME, 0, 2000); /* 2ms */
        if (retval != 0) {
          fotg210_err(fotg210, "port %d resume error %d\n", wIndex + 1, retval);
          goto error;
        }
    ...
    if ((temp & PORT_RESET) &&
      ...
      fotg210_writel(fotg210, temp & ~(PORT_RWC_BITS | PORT_RESET), status_reg);
      /* REVISIT:  some hardware needs 550+ usec to clear
       * this bit; seems too long to spin routinely...
       */
      retval = handshake(fotg210, status_reg, PORT_RESET, 0, 1000);
      if (retval != 0) {
    ...
    if ((temp & PORT_CONNECT) && test_bit(wIndex, &fotg210->companion_ports)) {
      temp &= ~PORT_RWC_BITS;
  ...
  case SetPortFeature:
    ...
    case USB_PORT_FEAT_RESET:
       ...
       */
      fotg210->reset_done[wIndex] = jiffies + msecs_to_jiffies(50);
      fotg210_writel(fotg210, temp, status_reg);
      break;
...
static struct fotg210_qh *fotg210_qh_alloc(struct fotg210_hcd *fotg210,
  ...
  if (!qh)
    goto done;
  qh->hw =
      (struct fotg210_qh_hw *)dma_pool_zalloc(fotg210->qh_pool, flags, &dma);
  if (!qh->hw)
    goto fail;
  qh->qh_dma = dma;
...
static void fotg210_mem_cleanup(struct fotg210_hcd *fotg210) {
  ...
  if (fotg210->periodic)
    dma_free_coherent(fotg210_to_hcd(fotg210)->self.controller,
                      fotg210->periodic_size * sizeof(u32), fotg210->periodic,
                      fotg210->periodic_dma);
...
static int fotg210_mem_init(struct fotg210_hcd *fotg210, gfp_t flags) {
  ...
  fotg210->qtd_pool = dma_pool_create(
      "fotg210_qtd", fotg210_to_hcd(fotg210)->self.controller,
      sizeof(struct fotg210_qtd), 32 /* byte alignment (for hw parts) */,
      4096 /* can't cross 4K */);
  if (!fotg210->qtd_pool)
    goto fail;

  /* QHs for control/bulk/intr transfers */
  fotg210->qh_pool = dma_pool_create(
      "fotg210_qh", fotg210_to_hcd(fotg210)->self.controller,
      sizeof(struct fotg210_qh_hw), 32 /* byte alignment (for hw parts) */,
      4096 /* can't cross 4K */);
  if (!fotg210->qh_pool)
    goto fail;
  ...
  fotg210->itd_pool = dma_pool_create(
      "fotg210_itd", fotg210_to_hcd(fotg210)->self.controller,
      sizeof(struct fotg210_itd), 64 /* byte alignment (for hw parts) */,
      4096 /* can't cross 4K */);
  if (!fotg210->itd_pool)
    goto fail;

  /* Hardware periodic table */
  fotg210->periodic = dma_alloc_coherent(
      fotg210_to_hcd(fotg210)->self.controller,
      fotg210->periodic_size * sizeof(__le32), &fotg210->periodic_dma, 0);
...
static int qtd_fill(struct fotg210_hcd *fotg210, struct fotg210_qtd *qtd,
  ...
  if (likely(len < count))         /* ... iff needed */
    ...
    for (i = 1; count < len && i < 5; i++) {
      ...
      qtd->hw_buf_hi[i] = cpu_to_hc32(fotg210, (u32)(addr >> 32));
      buf += 0x1000;
      if ((count + 0x1000) < len)
        count += 0x1000;
      else
...
static inline void qh_update(struct fotg210_hcd *fotg210, struct fotg210_qh *qh,
                             struct fotg210_qtd *qtd) {
  struct fotg210_qh_hw *hw = qh->hw;
...
static void qh_refresh(struct fotg210_hcd *fotg210, struct fotg210_qh *qh) {
  ...
  if (list_empty(&qh->qtd_list))
    ...
    qtd = list_entry(qh->qtd_list.next, struct fotg210_qtd, qtd_list);
    /*
...
static void fotg210_clear_tt_buffer(struct fotg210_hcd *fotg210,
                                    struct fotg210_qh *qh, struct urb *urb,
                                    u32 token) {
  ...
  if (urb->dev->tt && !usb_pipeint(urb->pipe) && !qh->clearing_tt) {
    ...
    dev_dbg(&tt->dev, "clear tt buffer port %d, a%d ep%d t%08x\n",
            urb->dev->ttport, urb->dev->devnum, usb_pipeendpoint(urb->pipe),
            token);

    if (urb->dev->tt->hub != fotg210_to_hcd(fotg210)->self.root_hub) {
...
static int qtd_copy_status(struct fotg210_hcd *fotg210, struct urb *urb,
  ...
  if (token & QTD_STS_HALT) {
    ...
    fotg210_dbg(fotg210, "dev%d ep%d%s qtd token %08x --> status %d\n",
...
static void fotg210_urb_done(struct fotg210_hcd *fotg210, struct urb *urb,
                             int status) __releases(fotg210->lock)
  ...
  fotg210_dbg(fotg210, "%s %s urb %p ep%d%s status %d len %d/%d\n", __func__,
              urb->dev->devpath, urb, usb_pipeendpoint(urb->pipe),
              usb_pipein(urb->pipe) ? "in" : "out", status, urb->actual_length,
...
rescan:
  ...
  retry_xacterr:
    if ((token & QTD_STS_ACTIVE) == 0) {
      ...
      if ((token & QTD_STS_HALT) != 0) {
        ...
        if ((token & QTD_STS_XACT) && QTD_CERR(token) == 0 &&
           ...
           */
          token &= ~QTD_STS_HALT;
          token |= QTD_STS_ACTIVE | (FOTG210_TUNE_CERR << 10);
    ...
    } else if (likely(!stopped && fotg210->rh_state >= FOTG210_RH_RUNNING)) {
      break;
      ...
      if (state == QH_STATE_IDLE &&
          cpu_to_hc32(fotg210, qtd->qtd_dma) == hw->hw_current) {
    ...
    if (last_status == -EINPROGRESS) {
       ...
       */
      if (unlikely(last_status != -EINPROGRESS && last_status != -EREMOTEIO)) {
...
static struct list_head *qh_urb_transaction(struct fotg210_hcd *fotg210,
  ...
  if (usb_pipecontrol(urb->pipe)) {
    /* SETUP pid */
    qtd_fill(fotg210, qtd, urb->setup_dma, sizeof(struct usb_ctrlrequest),
             token | (2 /* "setup" */ << 8), 8);
  ...
  for (;;) {
    ...
    this_qtd_len = qtd_fill(fotg210, qtd, buf, this_sg_len, token, maxpacket);
...
static struct fotg210_qh *qh_make(struct fotg210_hcd *fotg210, struct urb *urb,
  ...
  if (type == PIPE_INTERRUPT) {
    qh->usecs =
        NS_TO_US(usb_calc_bus_time(USB_SPEED_HIGH, is_input, 0, mult * maxp));
    qh->start = NO_FRAME;
    ...
    if (urb->dev->speed == USB_SPEED_HIGH) {
      ...
      qh->gap_uf = 1 + usb_calc_bus_time(urb->dev->speed, is_input, 0, maxp) /
                           (125 * 1000);
      ...
      qh->tt_usecs = NS_TO_US(
          think_time + usb_calc_bus_time(urb->dev->speed, is_input, 0, maxp));
...
static int submit_async(struct fotg210_hcd *fotg210, struct urb *urb,
...
#ifdef FOTG210_URB_TRACE
  {
    ...
    qtd = list_entry(qtd_list->next, struct fotg210_qtd, qtd_list);
    fotg210_dbg(fotg210, "%s %s urb %p ep%d%s len %d, qtd %p [qh %p]\n",
                __func__, urb->dev->devpath, urb, epnum & 0x0f,
                (epnum & USB_DIR_IN) ? "in" : "out",
...
static void scan_async(struct fotg210_hcd *fotg210) {
  ...
  if (check_unlinks_later && fotg210->rh_state == FOTG210_RH_RUNNING &&
      !(fotg210->enabled_hrtimer_events & BIT(FOTG210_HRTIMER_ASYNC_UNLINKS))) {
    fotg210_enable_event(fotg210, FOTG210_HRTIMER_ASYNC_UNLINKS, true);
    ++fotg210->async_unlink_cycle;
  }
}
...
static void periodic_unlink(struct fotg210_hcd *fotg210, unsigned frame,
  ...
  while (here.ptr && here.ptr != ptr) {
    prev_p = periodic_next_shadow(fotg210, prev_p, Q_NEXT_TYPE(fotg210, *hw_p));
    hw_p = shadow_next_periodic(fotg210, &here, Q_NEXT_TYPE(fotg210, *hw_p));
    here = *prev_p;
  }
   ...
   */
  *prev_p = *periodic_next_shadow(fotg210, &here, Q_NEXT_TYPE(fotg210, *hw_p));
  ...
  while (q->ptr) {
    ...
    case Q_TYPE_QH:
      ...
      if (hw->hw_info2 & cpu_to_hc32(fotg210, 1 << uframe))
        usecs += q->qh->usecs;
      /* ... or C-mask? */
      if (hw->hw_info2 & cpu_to_hc32(fotg210, 1 << (8 + uframe)))
        usecs += q->qh->c_usecs;
      hw_p = &hw->hw_next;
      q = &q->qh->qh_next;
      break;
...
static int tt_no_collision(struct fotg210_hcd *fotg210, unsigned period,
                           struct usb_device *dev, unsigned frame,
                           u32 uf_mask) {
  if (period == 0) /* error */
    return 0;
    ...
    while (here.ptr) {
      ...
      case Q_TYPE_QH:
        ...
        if (same_tt(dev, here.qh->dev)) {
          ...
          mask = hc32_to_cpu(fotg210, hw->hw_info2);
...
static void qh_link_periodic(struct fotg210_hcd *fotg210,
  ...
  dev_dbg(&qh->dev->dev, "link qh%d-%04x/%p start %d [%d/%d us]\n", period,
          hc32_to_cpup(fotg210, &qh->hw->hw_info2) & (QH_CMASK | QH_SMASK), qh,
          qh->start, qh->usecs, qh->c_usecs);
  ...
  fotg210_to_hcd(fotg210)->self.bandwidth_allocated +=
      qh->period ? ((qh->usecs + qh->c_usecs) / qh->period) : (qh->usecs * 8);
...
static void qh_unlink_periodic(struct fotg210_hcd *fotg210,
  ...
  dev_dbg(&qh->dev->dev, "unlink qh%d-%04x/%p start %d [%d/%d us]\n",
          qh->period,
          hc32_to_cpup(fotg210, &qh->hw->hw_info2) & (QH_CMASK | QH_SMASK), qh,
          qh->start, qh->usecs, qh->c_usecs);
  ...
  if (fotg210->qh_scan_next == qh)
    fotg210->qh_scan_next =
        list_entry(qh->intr_node.next, struct fotg210_qh, intr_node);
...
static int check_intr_schedule(struct fotg210_hcd *fotg210, unsigned frame,
  ...
  *c_maskp = cpu_to_hc32(fotg210, mask << 8);
  ...
  if (tt_no_collision(fotg210, qh->period, qh->dev, frame, mask)) {
    if (!check_period(fotg210, frame, uframe + qh->gap_uf + 1, qh->period,
                      qh->c_usecs))
      goto done;
    if (!check_period(fotg210, frame, uframe + qh->gap_uf, qh->period,
                      qh->c_usecs))
      goto done;
    retval = 0;
  }
...
static int qh_schedule(struct fotg210_hcd *fotg210, struct fotg210_qh *qh) {
  ...
  if (frame < qh->period) {
    ...
    status = check_intr_schedule(fotg210, frame, --uframe, qh, &c_mask);
  } else {
    uframe = 0;
    c_mask = 0;
    status = -ENOSPC;
  ...
  if (status) {
    ...
    hw->hw_info2 |= qh->period ? cpu_to_hc32(fotg210, 1 << uframe)
...
static void scan_intr(struct fotg210_hcd *fotg210) {
  ...
  rescan:
    ...
    if (!list_empty(&qh->qtd_list)) {
       ...
       */
      temp = qh_completions(fotg210, qh);
      if (unlikely(qh->needs_rescan || (list_empty(&qh->qtd_list) &&
                                        qh->qh_state == QH_STATE_LINKED)))
...
static struct fotg210_iso_stream *iso_stream_find(struct fotg210_hcd *fotg210,
  ...
  if (unlikely(stream == NULL)) {
    ...
    if (likely(stream != NULL)) {
      ep->hcpriv = stream;
      stream->ep = ep;
      iso_stream_init(fotg210, stream, urb->dev, urb->pipe, urb->interval);
    }
...
static int itd_urb_transaction(struct fotg210_iso_stream *stream,
                               struct fotg210_hcd *fotg210, struct urb *urb,
                               gfp_t mem_flags) {
  struct fotg210_itd *itd;
  dma_addr_t itd_dma;
  int i;
  ...
  for (i = 0; i < num_itds; i++) {
    ...
    if (likely(!list_empty(&stream->free_list))) {
      itd = list_first_entry(&stream->free_list, struct fotg210_itd, itd_list);
    ...
    alloc_itd:
      spin_unlock_irqrestore(&fotg210->lock, flags);
      itd = dma_pool_alloc(fotg210->itd_pool, mem_flags, &itd_dma);
      spin_lock_irqsave(&fotg210->lock, flags);
      if (!itd) {
...
static int iso_stream_schedule(struct fotg210_hcd *fotg210, struct urb *urb,
  ...
  if (likely(!list_empty(&stream->td_list))) {
    ...
    if (excess >= mod - 2 * SCHEDULE_SLOP)
      start = next + excess - mod + period * DIV_ROUND_UP(mod - excess, period);
    else
      start = next + excess + period;
    if (start - now >= mod) {
      fotg210_dbg(fotg210, "request %p would overflow (%d+%d >= %d)\n", urb,
                  start - now - period, period, mod);
      status = -EFBIG;
      goto fail;
    }
  }
   ...
   */
    ...
    do {
      ...
      if (itd_slot_ok(fotg210, mod, start, stream->usecs, period))
        done = 1;
  ...
  if (unlikely(start - now + span - period >= mod - 2 * SCHEDULE_SLOP)) {
    fotg210_dbg(fotg210, "request %p would overflow (%d+%d >= %d)\n", urb,
                start - now, span - period, mod - 2 * SCHEDULE_SLOP);
    status = -EFBIG;
    goto fail;
  }
...
static void itd_link_urb(struct fotg210_hcd *fotg210, struct urb *urb,
  ...
  if (unlikely(list_empty(&stream->td_list))) {
    fotg210_to_hcd(fotg210)->self.bandwidth_allocated += stream->bandwidth;
    fotg210_dbg(fotg210, "schedule devp %s ep%d%s-iso period %d start %d.%d\n",
                urb->dev->devpath, stream->bEndpointAddress & 0x0f,
                (stream->bEndpointAddress & USB_DIR_IN) ? "in" : "out",
                urb->interval, next_uframe >> 3, next_uframe & 0x7);
  }
  ...
  for (packet = 0, itd = NULL; packet < urb->number_of_packets;) {
    if (itd == NULL) {
      ...
      itd = list_entry(iso_sched->td_list.next, struct fotg210_itd, itd_list);
...
#define ISO_ERRS                                                               \
  (FOTG210_ISOC_BUF_ERR | FOTG210_ISOC_BABBLE | FOTG210_ISOC_XACTERR)
...
static bool itd_complete(struct fotg210_hcd *fotg210, struct fotg210_itd *itd) {
  ...
  for (uframe = 0; uframe < 8; uframe++) {
    ...
    if (unlikely(t & ISO_ERRS)) {
      ...
      if (t & FOTG210_ISOC_BUF_ERR)
        desc->status = usb_pipein(urb->pipe) ? -ENOSR  /* hc couldn't read */
  ...
  if (unlikely(list_is_singular(&stream->td_list))) {
    fotg210_to_hcd(fotg210)->self.bandwidth_allocated -= stream->bandwidth;
    fotg210_dbg(fotg210, "deschedule devp %s ep%d%s-iso\n", dev->devpath,
...
#ifdef FOTG210_URB_TRACE
  fotg210_dbg(fotg210, "%s %s urb %p ep%d%s len %d, %d pkts %d uframes[%p]\n",
              __func__, urb->dev->devpath, urb, usb_pipeendpoint(urb->pipe),
              usb_pipein(urb->pipe) ? "in" : "out", urb->transfer_buffer_length,
              urb->number_of_packets, urb->interval, stream);
#endif
  ...
  while (q.ptr) {
    ...
    case Q_TYPE_ITD:
      ...
      if (frame == now_frame && live) {
        rmb();
        for (uf = 0; uf < 8; uf++) {
          if (q.itd->hw_transaction[uf] & ITD_ACTIVE(fotg210))
            break;
        }
        if (uf < 8) {
          q_p = &q.itd->itd_next;
...
static void scan_isoc(struct fotg210_hcd *fotg210) {
  ...
  for (;;) {
    ret = 1;
    while (ret != 0)
      ret = scan_frame_queue(fotg210, frame, now_frame, live);
...
static ssize_t uframe_periodic_max_store(struct device *dev,
  ...
  if (uframe_periodic_max < fotg210->uframe_periodic_max) {
    ...
    for (frame = 0; frame < fotg210->periodic_size; ++frame)
      for (uframe = 0; uframe < 7; ++uframe)
        allocated_max =
            max(allocated_max, periodic_usecs(fotg210, frame, uframe));

    if (allocated_max > uframe_periodic_max) {
      fotg210_info(fotg210,
                   "cannot decrease uframe_periodic_max because periodic "
                   "bandwidth is already allocated (%u > %u)\n",
...
static int fotg210_run(struct usb_hcd *hcd) {
  ...
  temp =
      HC_VERSION(fotg210, fotg210_readl(fotg210, &fotg210->caps->hc_capbase));
  fotg210_info(fotg210, "USB %x.%x started, EHCI %x.%02x\n",
               ((fotg210->sbrn & 0xf0) >> 4), (fotg210->sbrn & 0x0f), temp >> 8,
               temp & 0xff);
...
static irqreturn_t fotg210_irq(struct usb_hcd *hcd) {
  ...
  if (status & STS_PCD) {
    ...
    if (test_bit(0, &fotg210->suspended_ports) &&
        ...
        (pstatus & PORT_PE) && fotg210->reset_done[0] == 0) {
...
rescan:
  ...
  case QH_STATE_LINKED:
  case QH_STATE_COMPLETING:
    for (tmp = fotg210->async->qh_next.qh; tmp && tmp != qh;
         tmp = tmp->qh_next.qh)
      continue;
  ...
  default:
    ...
    fotg210_err(fotg210, "qh %p (#%02x) state %d%s\n", qh,
                ep->desc.bEndpointAddress, qh->qh_state,
...
int fotg210_hcd_probe(struct platform_device *pdev, struct fotg210 *fotg) {
  ...
  hcd = usb_create_hcd(&fotg210_fotg210_hc_driver, dev, dev_name(dev));
  if (!hcd) {
...
int __init fotg210_hcd_init(void) {
  ...
  if (test_bit(USB_UHCI_LOADED, &usb_hcds_loaded) ||
    ...
    pr_warn("Warning! fotg210_hcd should always be loaded before uhci_hcd and "
            "ohci_hcd, not after\n");
