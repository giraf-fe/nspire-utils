...
static const char *const fotg210_ep_name[] = {"ep0", "ep1", "ep2", "ep3",
                                              "ep4"};
...
static void fotg210_set_mps(struct fotg210_ep *ep, u32 epnum, u32 mps,
  ...
  struct fotg210_udc *fotg210 = ep->fotg210;
  u32 val;
  u32 offset = dir_in ? FOTG210_INEPMPSR(epnum) : FOTG210_OUTEPMPSR(epnum);
...
static void fotg210_reset_tseq(struct fotg210_udc *fotg210, u8 epnum) {
  ...
  reg = (ep->dir_in) ? fotg210->reg + FOTG210_INEPMPSR(epnum)
...
static void fotg210_enable_dma(struct fotg210_ep *ep, dma_addr_t d, u32 len) {
  u32 value;
  struct fotg210_udc *fotg210 = ep->fotg210;
...
static void fotg210_wait_dma_done(struct fotg210_ep *ep) {
  ...
  do {
    ...
    if ((value & DISGR2_USBRST_INT) || (value & DISGR2_DMA_ERROR))
      goto dma_reset;
...
dma_reset:
  ...
  /* reset fifo */
  if (ep->epnum) {
    value = ioread32(ep->fotg210->reg + FOTG210_FIBCR(ep->epnum - 1));
    value |= FIBCR_FFRST;
    iowrite32(value, ep->fotg210->reg + FOTG210_FIBCR(ep->epnum - 1));
  } else {
...
static void fotg210_start_dma(struct fotg210_ep *ep,
  ...
  if (ep->epnum) {
    if (ep->dir_in) {
      ...
      length = ioread32(ep->fotg210->reg + FOTG210_FIBCR(ep->epnum - 1)) &
               FIBCR_BCFX;
...
static void fotg210_set_epnstall(struct fotg210_ep *ep) {
  ...
  reg = (ep->dir_in) ? fotg210->reg + FOTG210_INEPMPSR(ep->epnum)
...
static void fotg210_clear_tx0byte(struct fotg210_udc *fotg210) {
  ...
  value &= ~(TX0BYTE_EP1 | TX0BYTE_EP2 | TX0BYTE_EP3 | TX0BYTE_EP4);
...
static void fotg210_clear_rx0byte(struct fotg210_udc *fotg210) {
  ...
  value &= ~(RX0BYTE_EP1 | RX0BYTE_EP2 | RX0BYTE_EP3 | RX0BYTE_EP4);
  iowrite32(value, fotg210->reg + FOTG210_RX0BYTE);
}

/* read 8-byte setup packet only */
static void fotg210_rdsetupp(struct fotg210_udc *fotg210, u8 *buffer) {
  int i = 0;
  u8 *tmp = buffer;
  u32 data;
  u32 length = 8;
...
static void fotg210_clear_feature(struct fotg210_udc *fotg210,
                                  struct usb_ctrlrequest *ctrl) {
  struct fotg210_ep *ep = fotg210->ep[ctrl->wIndex & USB_ENDPOINT_NUMBER_MASK];
...
static void fotg210_get_status(struct fotg210_udc *fotg210,
  ...
  case USB_RECIP_ENDPOINT:
    ...
    if (epnum)
      fotg210->ep0_data = cpu_to_le16(fotg210_is_epnstall(fotg210->ep[epnum])
                                      << USB_ENDPOINT_HALT);
    else
      fotg210_request_error(fotg210);
    break;
...
static void fotg210_in_fifo_handler(struct fotg210_ep *ep) {
  struct fotg210_request *req =
      list_entry(ep->queue.next, struct fotg210_request, queue);
...
static irqreturn_t fotg210_irq(int irq, void *_fotg210) {
  ...
  if (int_grp & DIGR_INT_G2) {
    ...
    if (int_grp2 & DISGR2_USBRST_INT) {
      usb_gadget_udc_reset(&fotg210->gadget, fotg210->driver);
  ...
  if (int_grp & DIGR_INT_G0) {
    ...
    if (int_grp0 & DISGR0_CX_SETUP_INT) {
      if (fotg210_setup_packet(fotg210, &ctrl)) {
        spin_unlock(&fotg210->lock);
        if (fotg210->driver->setup(&fotg210->gadget, &ctrl) < 0)
          fotg210_set_cxstall(fotg210);
        spin_lock(&fotg210->lock);
      }
    }
...
static int fotg210_udc_start(struct usb_gadget *g,
  ...
  if (!IS_ERR_OR_NULL(fotg210->phy)) {
    ret = otg_set_peripheral(fotg210->phy->otg, &fotg210->gadget);
    if (ret)
...
int fotg210_udc_probe(struct platform_device *pdev, struct fotg210 *fotg) {
  ...
  for (i = 0; i < FOTG210_MAX_NUM_EP; i++) {
    ...
    if (i) {
      ...
      list_add_tail(&fotg210->ep[i]->ep.ep_list, &fotg210->gadget.ep_list);
    }
    ep->fotg210 = fotg210;
  ...
  fotg210->ep0_req = fotg210_ep_alloc_request(&fotg210->ep[0]->ep, GFP_KERNEL);
  ...
  ret = request_irq(irq, fotg210_irq, IRQF_SHARED, udc_name, fotg210);
  if (ret < 0) {
