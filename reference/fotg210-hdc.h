...
struct fotg210_caps {
   ...
   */
  u32 hc_capbase;
#define HC_LENGTH(fotg210, p)                                                  \
  (0x00ff & ((p) >> /* bits 7:0 / offset 00h */                                \
             (fotg210_big_endian_capbase(fotg210) ? 24 : 0)))
#define HC_VERSION(fotg210, p)                                                 \
  (0xffff & ((p) >> /* bits 31:16 / offset 02h */                              \
...
#define HCC_PGM_FRAMELISTLEN(p)                                                \
  ((p) & (1 << 1)) /* true: periodic_size changes*/
...
static inline unsigned int fotg210_get_speed(struct fotg210_hcd *fotg210,
                                             unsigned int portsc) {
  return (readl(&fotg210->regs->otgcsr) & OTGCSR_HOST_SPD_TYP) >> 22;
}
